...
namespace EsentLib.Jet
{
    ...
    /// defragmentation, backup, or restore operations. During such operations, the database
    /// engine calls this callback function to give an update on the progress of the operation.
    /// </summary>
    /// <param name="sesid">The session with which the long running operation was called.</param>
    ...
    /// engine calls this callback function to give an update on the progress of the operation.
    /// </summary>
    /// <remarks>This is the internal version of the callback. The final parameter is a void*
    /// pointer, which may point to a NATIVE_SNPROG.</remarks>
    /// <param name="nativeSesid">The session with which the long running operation was called.
    /// </param>
    /// <param name="snp">The type of operation.</param>
    /// <param name="snt">The status of the operation.</param>
    ...
    /// <summary>Wraps a NATIVE_PFNSTATUS callback around a JET_PFNSTATUS. This is used to convert
    /// the snprog argument to a managed snprog.</summary>
    internal sealed class StatusCallbackWrapper
    {
        /// <summary>Initializes static members of the <see cref="StatusCallbackWrapper"/> class.
        /// </summary>
        static StatusCallbackWrapper()
        {
            ...
            RuntimeHelpers.PrepareMethod(typeof(StatusCallbackWrapper).GetMethod("CallbackImpl",
                BindingFlags.NonPublic | BindingFlags.Instance).MethodHandle);
        }

        /// <summary>Initializes a new instance of the StatusCallbackWrapper class.</summary>
        /// <param name="wrappedCallback">The managed callback to use.</param>
        ...
        /// <summary>Gets a NATIVE_PFNSTATUS callback that wraps the managed callback.</summary>
        public NATIVE_PFNSTATUS NativeCallback
        {
            get { return this.nativeCallback; }
        }

        /// <summary>Gets or sets the saved exception. If the callback throws an exception it is
        /// saved here and should be rethrown when the API call finishes.</summary>
        private Exception SavedException { get; set; }

        /// <summary>Gets or sets a value indicating whether the thread was aborted during the
        /// callback.</summary>
        private bool ThreadWasAborted { get; set; }

        /// <summary>If an exception was generated during a callback throw it.</summary>
        public void ThrowSavedException()
        {
            if (this.ThreadWasAborted) { Thread.CurrentThread.Abort(); }
            if (null != this.SavedException) { throw this.SavedException; }
        }

        /// <summary>Callback function for native code. We don't want to throw an exception
        /// through unmanaged ESENT because that will corrupt ESENT's internal state. Instead
        /// we catch all exceptions and return an error instead. We use a CER to make catching
        /// the exceptions as reliable as possible.</summary>
        /// <param name="nativeSesid">The session with which the long running operation was
        /// called.</param>
        ...
        private static readonly TraceSwitch TraceSwitch =
            new TraceSwitch("ESENT StatusCallbackWrapper", "Wrapper around unmanaged ESENT status callback");
        /// <summary>The wrapped status callback.</summary>
        private readonly JET_PFNSTATUS wrappedCallback;
        /// <summary>The native version of the callback. This will be a closure (because we are
        /// wrapping a non-static method) so keep track of it here to make sure it isn't garbage
        /// collected.</summary>
