...
namespace EsentLib
{
    /// <summary>Base class for enumerators that return ColumnInfo objects. Subclasses differ
    /// by how they open the table.</summary>
    internal abstract class ColumnInfoEnumerator : TableEnumerator<ColumnInfo>
    {
        /// <summary>Initializes a new instance of the <see cref="ColumnInfoEnumerator"/> class.</summary>
        /// <param name="sesid">The session to use.</param>
        protected ColumnInfoEnumerator(JET_SESID sesid) : base(sesid)
        {            
        }

        /// <summary>Gets or sets the columnlist used to retrieve data.</summary>
        ...
        /// <summary>Create a ColumnInfo object from the data in the current JET_COLUMNLIST
        /// entry.</summary>
        /// <param name="sesid">The session to use.</param>
        ...
        private static ColumnInfo GetColumnInfoFromColumnlist(JET_SESID sesid, JET_COLUMNLIST columnlist)
        {
            ...
            string name = Api.RetrieveColumnAsString(sesid, columnlist.tableid, columnlist.columnidcolumnname,
                encodingOfTextColumns, RetrieveColumnGrbit.None);
            ...
            return new ColumnInfo(name, new JET_COLUMNID { Value = columnidValue },
                (JET_coltyp)coltypValue, (JET_CP)codepageValue, unchecked((int)maxLength),
                defaultValue, (ColumndefGrbit)grbitValue);
        }
    }
}
