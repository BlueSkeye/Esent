...
namespace EsentLib
{
    /// <summary>Provide methods to convert data and flags between Win32 and the .NET Framework.</summary>
    public static class Conversions
    {
        /// <summary>Initializes static members of the Conversions class. This sets up the
        /// conversion mapping dictionaries.</summary>
        static Conversions()
        {
            ...
            CompareOptionsToLcmapFlags = new Dictionary<CompareOptions, uint> {
                { CompareOptions.IgnoreCase, NORM_IGNORECASE },
                { CompareOptions.IgnoreKanaType, NORM_IGNOREKANATYPE },
                { CompareOptions.IgnoreNonSpace, NORM_IGNORENONSPACE },
                { CompareOptions.IgnoreSymbols, NORM_IGNORESYMBOLS },
                { CompareOptions.IgnoreWidth, NORM_IGNOREWIDTH },
                { CompareOptions.StringSort, SORT_STRINGSORT }
            };
            LcmapFlagsToCompareOptions = InvertDictionary(CompareOptionsToLcmapFlags);
        }

        /// <summary>Convert a double (OA date time format) to a DateTime. Unlike
        /// DateTime.FromOADate this doesn't throw exceptions.</summary>
        /// <param name="d">The double value.</param>
        /// <returns>A DateTime.</returns>
        public static DateTime ConvertDoubleToDateTime(double d)
        {
            try { return LibraryHelpers.FromOADate(d); }
        ...
        /// <summary>Given flags for LCMapFlags, turn them into compare options. Unknown
        /// options  are ignored.</summary>
        ...
        {
            // This should be a template, but there isn't an elegant way to express than with
            // C# generics
        ...
        /// <summary>Give CompareOptions, turn them into flags from LCMapString. Unknown options
        /// are ignored.</summary>
        /// <param name="compareOptions">The options to convert.</param>
        /// <returns>The LCMapString flags that match the compare options. Unsupported options
        /// are ignored.</returns>
        [CLSCompliant(false)]
        ...
        /// <summary>Given a Key=>Value dictionary create an inverted dictionary that maps
        /// Value=>Key.</summary>
        ...
        private static IDictionary<TKey, TValue> InvertDictionary<TValue, TKey>(
            ICollection<KeyValuePair<TValue, TKey>> dict)
        {
            Dictionary<TKey, TValue> result = new Dictionary<TKey, TValue>(dict.Count);
        ...
        // Unmanaged constants used in the conversion.
        ...
        /// <summary>Produce a normalized wide-character sort key.</summary>
        private const uint LCMAP_SORTKEY = 0x00000400;
        /// <summary>Maps a CompareOption enumeration to the corresponding LCMapString flag.</summary>
        private static readonly IDictionary<CompareOptions, uint> CompareOptionsToLcmapFlags;
        /// <summary>Maps an LCMapString flag to the corresponding CompareOption enumeration.</summary>
