...
{
    /// <summary>Base class for objects that represent a column value to be set.</summary>
    public abstract partial class ColumnValue
    {
        /// <summary>Gets or sets the columnid to be set or retrieved.</summary>
        public JET_COLUMNID Columnid { get; set; }

        /// <summary>Initializes a new instance of the ColumnValue class.</summary>
        protected ColumnValue()
        {
            this.ItagSequence = 1;
        }

        /// <summary>Gets the warning generated by retrieving or setting this column.</summary>
        public JET_wrn Error { get; internal set; }

        /// <summary>Gets or sets the column itag sequence.</summary>
        public int ItagSequence { get; set; }

        /// <summary>Gets the byte length of a column value, which is zero if column is null,
        /// otherwise it matches the Size for fixed-size columns and represent the actual value
        /// byte length for variable sized columns (i.e. binary and string). For strings the
        /// length is determined in assumption two bytes per character.</summary>
        public abstract int Length { get; }

        /// <summary>Gets or sets column retrieval options.</summary>
        ...
        /// <summary>Gets or sets column update options.</summary>
        ...
        /// <summary>Create a native RetrieveColumn from this object.</summary>
        ...
        /// <summary>Create a native SetColumn from this object.</summary>
        ...
        /// <summary>Recursive RetrieveColumns method for data pinning. This should pin a buffer
        /// and call the inherited RetrieveColumns method.</summary>
        /// <param name="sesid">The session to use.</param>
        /// <param name="tableid">The table to retrieve the columns from.</param>
        /// <param name="columnValues">Column values to retrieve.</param>
        internal static void RetrieveColumns(JET_SESID sesid, JET_TABLEID tableid,
            ColumnValue[] columnValues)
        {
            const int MaxColumnValues = 1024;
            if (columnValues.Length > MaxColumnValues) {
                throw new ArgumentOutOfRangeException("columnValues", columnValues.Length,
                    "Too many column values");    
            }
            unsafe {
                byte[] buffer = null;
                NATIVE_RETRIEVECOLUMN* nativeRetrievecolumns =
                    stackalloc NATIVE_RETRIEVECOLUMN[columnValues.Length];

                try {
                    buffer = MemoryCache.ColumnCache.Allocate();
                    Debug.Assert(MaxColumnValues * 16 < buffer.Length,
                        "Maximum size of fixed columns could exceed buffer size");
                    fixed (byte* pinnedBuffer = buffer) {
                        byte* currentBuffer = pinnedBuffer;
                        int numVariableLengthColumns = columnValues.Length;
                        // First the fixed-size columns
                        for (int i = 0; i < columnValues.Length; ++i) {
                            if (0 == columnValues[i].Size) { continue; }
                            ...
                            Debug.Assert(currentBuffer <= pinnedBuffer + buffer.Length,
                                "Moved past end of pinned buffer");
                            numVariableLengthColumns--;
                        }

                        // Now the variable-length columns
                        if (0 < numVariableLengthColumns) {
                            ...
                            Debug.Assert(0 < bufferPerColumn,
                                "Not enough buffer left to retrieve variable length columns");
                        ...
                        EsentExceptionHelper.Check(LegacyApi.Impl.JetRetrieveColumns(sesid, tableid,
                            nativeRetrievecolumns, columnValues.Length));
                        ...
                        for (int i = 0; i < columnValues.Length; ++i) {
                            if (nativeRetrievecolumns[i].err == (int)JET_wrn.BufferTruncated) { continue; }
                            byte* columnBuffer = (byte*)nativeRetrievecolumns[i].pvData;
                            int startIndex = checked((int)(columnBuffer - pinnedBuffer));
                            columnValues[i].GetValueFromBytes(buffer, startIndex,
        ...
        /// <summary>Retrieve the value for columns whose buffers were truncated.</summary>
        /// <param name="sesid">The session to use.</param>
        ...
        /// </param>
        private static unsafe void RetrieveTruncatedBuffers(JET_SESID sesid, JET_TABLEID tableid,
            IList<ColumnValue> columnValues, NATIVE_RETRIEVECOLUMN* nativeRetrievecolumns)
        {
            for (int i = 0; i < columnValues.Count; ++i) {
                ...
                fixed (byte* pinnedBuffer = buffer) {
                    ...
                    //err = LegacyApi.Impl.JetRetrieveColumn(sesid, tableid, columnValues[i].Columnid,
                    //    new IntPtr(pinnedBuffer), buffer.Length, out actualSize,
                    //    columnValues[i].RetrieveGrbit, retinfo);
                }
        ...
        /// <summary>Recursive SetColumns method for data pinning. This should populate 
        /// buffer and call the inherited SetColumns method.</summary>
        /// <param name="sesid">The session to use.</param>
        ...
        internal abstract unsafe int SetColumns(JET_SESID sesid, JET_TABLEID tableid,
            ColumnValue[] columnValues, NATIVE_SETCOLUMN* nativeColumns, int i);

        /// <summary>Recursive SetColumns function used to pin data.</summary>
        /// <param name="sesid">The session to use.</param>
        /// <param name="tableid">The table to set the columns in. An update should be prepared.</param>
        /// <param name="columnValues">Column values to set.</param>
        /// <param name="nativeColumns">Structures to put the pinned data in.</param>
        ...
        /// <returns>An error code.</returns>
        /// <remarks>This is marked as internal because it uses the NATIVE_SETCOLUMN type
        /// which is also marked as internal. It should be treated as a protected method
        /// though.</remarks>
        internal unsafe int SetColumns(JET_SESID sesid, JET_TABLEID tableid, ColumnValue[] columnValues,
            NATIVE_SETCOLUMN* nativeColumns, int i, void* buffer, int bufferSize, bool hasValue)
        {
        ...
        /// <see cref="ColumnValue"/>.</summary>
        /// <returns>A <see cref="T:System.String"/> that represents the current
        /// <see cref="ColumnValue"/>.</returns>
        public abstract override string ToString();

        /// <summary>Validation for the requested retrieve options for the column.</summary>
        ...
        /// <summary>Internal grbit.</summary>
